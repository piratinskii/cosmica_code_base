#!/usr/bin/env python3
"""
MobileNetV3-FasterRCNN-FPN Training Script for COSMICA Dataset

Trains MobileNetV3-FasterRCNN-FPN model for astronomical object detection.
Uses COCO format annotations with efficient mobile architecture.

Usage: python fasterrcnn_training.py --data /path/to/coco/dataset --epochs 100
"""
...
class COCODataset(Dataset):
    """Custom COCO Dataset for FasterRCNN training"""
    ...
    def __init__(self, root_dir, annotation_file, transforms=None):
        ...
        # Create category mapping (FasterRCNN uses 1-based class indexing)
        self.cat_ids = {cat['id']: i+1 for i, cat in enumerate(self.coco_data['categories'])}
    ...
    def __getitem__(self, idx):
        ...
        if len(boxes) == 0:
            boxes = [[0, 0, 1, 1]]
            labels = [1]  # Background class for FasterRCNN
        ...
        # Create target dictionary for FasterRCNN
        target = {
            "boxes": boxes,
            "labels": labels,
    ...
    def get_num_classes(self):
        # FasterRCNN includes background class, so +1
        return len(self.coco_data['categories']) + 1
...
def get_fasterrcnn_model(num_classes):
    """Create MobileNetV3-FasterRCNN-FPN model"""
...
def main():
    parser = argparse.ArgumentParser(description='Train MobileNetV3-FasterRCNN-FPN on COSMICA dataset')
    ...
    parser.add_argument('--comet-project', default='fasterrcnn-cosmica', help='CometML project name')
    ...
    # Create datasets
    train_dataset = COCODataset(train_dir, train_anno, transforms=get_transforms(True))
    valid_dataset = COCODataset(valid_dir, valid_anno, transforms=get_transforms(False))
    ...
    for epoch in range(args.epochs):
        ...
        if map_score > best_map:
            best_map = map_score
            patience_counter = 0
            torch.save(model.state_dict(), f'best_fasterrcnn_map_{map_score:.4f}.pth')
